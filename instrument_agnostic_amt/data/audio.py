...
def load_audio_window(
    ...
    """Load a fixed audio window as stereo float32 [channels, frames]."""
    ...
    # Keep stereo input and duplicate mono input to two channels.
    if audio.shape[1] > 2:
        audio = audio[:, :2]
    elif audio.shape[1] == 1:
        audio = np.repeat(audio, 2, axis=1)

    audio = audio.transpose(1, 0)  # [channels, frames]
