...
| 日付 | 内容 |
|---|---|
| 2026-07-15 | 🎸 ベースモデル v2（`--type bass_v2`）を追加。ノート検出精度と、楽器分類におけるスラップベースの分類精度が向上しました。<br>⚠️ 学習データ作成に使用したアノテーションツールの不具合により onset がずれ、精度低下を招いていたことが判明しました。ツールは修正済みで、修正後のデータを用いた再学習を予定しています。 |
