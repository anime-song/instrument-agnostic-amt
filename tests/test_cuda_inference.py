...
def test_core_amt_forward_runs_on_cuda(amp: bool) -> None:
    ...
    waveform = torch.randn(1, 2, 4_096, device="cuda")
    valid_frames = torch.tensor([4_096], device="cuda")
...
def test_core_amt_cuda_matches_cpu_in_fp32() -> None:
    ...
    waveform = torch.randn(1, 2, 4_096)
    valid_frames = torch.tensor([4_096])
...
def test_core_amt_compiled_forward_runs_on_cuda() -> None:
    ...
    for amp in (False, True):
        ...
        compiled_outputs = _run_core_forward(
            compiled_forward,
            waveform,
            valid_frames,
            amp=amp,
        ...
        tensors = [
            value
            for value in compiled_outputs.values()
...
def test_velocity_compiled_forward_handles_varying_shapes_on_cuda() -> None:
